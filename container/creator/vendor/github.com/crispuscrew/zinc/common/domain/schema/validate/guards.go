...
// ':'-delimited mount spec, where a stray space/comma/':' shifts fields (§5.5).
...
// must not smuggle extra FROM-line directives (§5.5).
...
// LocalImage reports a localhost/ image — the only refs exempt from the §5.5 digest
