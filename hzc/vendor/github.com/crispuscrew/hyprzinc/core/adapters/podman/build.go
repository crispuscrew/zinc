...
// image (FROM app.image + the install layer) and reads back the fingerprint label.
type Builder struct{}
...
func ImageBuildArgs(cfg domain.AppConfig) []string {
	return []string{
		"build",
		"-t", domain.DerivedImageRef(cfg.App.Name),
		"--label", domain.BuildLabel + "=" + domain.BuildFingerprint(cfg),
...
// line is debuggable.
func (Builder) Build(cfg domain.AppConfig) error {
	proc := exec.Command("podman", ImageBuildArgs(cfg)...)
	proc.Stdin = strings.NewReader(domain.DerivedContainerfile(cfg))
	if out, err := proc.CombinedOutput(); err != nil {
		return fmt.Errorf("build image for %s: %s", cfg.App.Name, strings.TrimSpace(string(out)))
	}
	return nil
}
...
// is empty for an image built outside HyprZinc, which also triggers a rebuild.
func (Builder) Fingerprint(ref string) (string, error) {
	out, err := exec.Command("podman", "image", "inspect", ref,
		"--format", fmt.Sprintf(`{{index .Config.Labels %q}}`, domain.BuildLabel)).Output()
	if err != nil {
		return "", err
	}
